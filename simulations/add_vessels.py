...
def main(cfg):
    ...
    coords = np.load( open(pj(cfg["vessels_dir"], "synet_coords.npy"), "rb") )
    edges = np.load( open(pj(cfg["vessels_dir"], "synet_edges.npy"), "rb") )
...
#        thickness = int( max(1, 10 * (radii[edge[0]] + radii[edge[1]]) / 2 ) )
        ...
        cv2.line(blank2,  p0[:2], p1[:2], color=(127,32,96),
                thickness=1)
#        cv2.line(blank,  [p0[1], p1[1]], [p0[2], p1[2]], color=(127,32,96),
#                thickness=1)
#        if e_i == 100:
#            break
...
if __name__ == "__main__":
    ...
    parser.add_argument("-b", "--blank", type=str,
            default=pj(HOME,
                "Datasets/Retina/DRIVE/training/blanks/22_blank.png"))
